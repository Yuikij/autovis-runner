...
import type { LiveViewportState } from "./run.js"
...
export interface AuthProfile {
  ...
  /**
   * 前置登录态（多选）：手动登录沙盒启动时，先把这些 profile 已保存的 storageState
   * （cookies + localStorage）注入浏览器，再打开目标站。用于"站点登录依赖第三方登录"
   * 的场景——如目标站用 Google/GitHub OAuth 登录，注入后点"用 Google 登录"直接带上会话。
   * 按数组顺序注入，后面的覆盖前面的同名 cookie。
   */
  prerequisiteAuthProfileIds?: Identifier[]
  /**
   * 持久 profile 模式（默认开）：回放与登录沙盒共用磁盘上的固定 userDataDir，
   * cookie/缓存/指纹/cf_clearance 跨运行保留——专治 Cloudflare 人机质询这类"风险分"拦截。
   * 仅当需要同一登录态并发跑多个任务时才关掉（持久目录一次只能一个 Chrome 用）。
   */
...
export interface AuthLoginSandboxSession {
  ...
  postLoginUrl?: string
  /** 启动时注入的前置登录态统计（未配置前置或全部无 state 时缺省）。 */
  prerequisiteInjection?: {
    profileNames: string[]
    cookieCount: number
    originCount: number
  }

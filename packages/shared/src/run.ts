...
import type { TaskModeConfig, TaskRunStatus } from "./task.js"
...
export interface ExecutionRun {
  ...
  /**
   * API 调用的结构化响应体：目标脚本通过 `result.set()` 写入，
   * 按用例 contract 的 response schema 校验后由 API 网关返回给调用方。
   * 仅在以 API 方式调用用例时产生（区别于 `runtimeOutputs` 的链路传递 + UI 展示）。
   */

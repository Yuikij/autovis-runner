# Browsewright 登录态采集器（Chrome 插件）

在**你自己的真浏览器**里采集目标站点的登录态（cookies + localStorage），一键导入 Browsewright 的
...
所以在本机采集登录态，再注入回 Browsewright，是对这类站点最可靠的路径（绕开 punish，而不是去打赢它）。
...
3. 点「加载已解压的扩展程序 / Load unpacked」，选择本目录
   `browsewright-runner/extensions/storagestate-importer/`

## 使用

1. 在本浏览器里**正常登录**目标站点（如 epicgames / 淘宝），并停在登录后的页面上。
2. 点插件图标，填 **Browsewright 运行机地址**（如 `http://192.168.1.10:8787`），点「连接」。
   - 若服务端开了鉴权：先在**同一个浏览器**里登录一下 Browsewright（之后免填 Token），
     或在「会话 Token」栏填入登录 Browsewright 后的会话值，再点连接。
3. 连接成功后，**项目 / 登录状态 Profile / 目标网址**都变成下拉框，直接选即可（不用再抄 ID）。
   - 目标网址留空＝写到项目主域名行。
   - 采集 cookie 的域名已按当前标签页自动填好，一般不用改。
4. 点「采集当前页面登录态并导入」。成功后回 Browsewright 用「检查登录状态」验证即可。
...
- 插件需要 `cookies` 权限和 `<all_urls>` 主机权限（跨域名采 cookie + 往运行机直传）。
  仅本地加载、不上架，权限只作用于你本机。
- 鉴权部署：若服务端开了鉴权，插件会把你填的 Token 作为 `browsewright_session` cookie 写到运行机域名后再
  带 `credentials` 提交。Token 即你登录 Browsewright 后的会话值。

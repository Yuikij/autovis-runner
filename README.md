...
There are three supported ways to deploy the runner:

| Method | Platforms | Service / autostart |
| --- | --- | --- |
| Install script | macOS, Linux, Windows | launchd / systemd / Windows Service (NSSM) |
...
boot, restarts on crash, and rotates logs. Useful options:

```powershell
# Build from the current source checkout instead of downloading a release:
...
# Files and deps only, no Windows Service:
...
C:\autovis-runner\winsw\autovis-service.exe start
C:\autovis-runner\winsw\autovis-service.exe stop
C:\autovis-runner\winsw\autovis-service.exe restart
...
With Docker Compose (recommended; see `docker-compose.yml` in this repo):

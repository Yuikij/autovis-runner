...
type BrowserFrameProps = {
  ...
  /** 全屏视图里悬浮在画面上方的内容（如可折叠状态栏）。 */
  fullscreenExtras?: React.ReactNode
  /** 提供后，头部全屏按钮改为调用该回调（由外层容器接管全屏，例如沙盒整体全屏）。 */
...
export function BrowserFrame({
  ...
  const drawFrame = useCallback((image: HTMLImageElement) => {
    for (const canvas of [inlineCanvasRef.current, fullscreenCanvasRef.current]) {
      if (!canvas) continue
      const context = canvas.getContext("2d")
      if (!context) continue
      canvas.width = frameSizeRef.current.width ?? image.width
      canvas.height = frameSizeRef.current.height ?? image.height
  ...
  useEffect(() => {
    ...
    frameSizeRef.current = { width: liveViewport.width, height: liveViewport.height }
  ...
  }, [liveViewport?.url, liveViewport?.status, liveViewport?.mimeType, liveViewport?.width, liveViewport?.height, drawFrame])

  // 进入全屏：调用浏览器原生 Fullscreen API 做真全屏；用户按 Esc 或浏览器退出时同步关闭。
  useEffect(() => {
  ...
  const fullscreenOverlay = isFullscreen ? (
    <div
      ...
      className="fixed inset-0 z-[100] flex flex-col bg-background dark:bg-slate-950 p-4 select-none animate-in fade-in duration-200"
    >
      {/* Header */}

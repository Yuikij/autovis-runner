...
export function AuthSandboxModal({
  ...
  const startPromiseRef = useRef<Promise<{ data: AuthLoginSandboxSession }> | null>(null)
  const startKeyRef = useRef<string | null>(null)
  const startDisposedRef = useRef(false)
  ...
  // 启动会话。dev 下 React StrictMode 会把挂载 effect 跑两遍：老写法（每次执行各发一个
  // 创建请求 + 闭包 cancelled 标志收尾）会让两个创建请求并发打到服务端，抢同一个持久
  // profile 目录——先到的在 goto 目标站期间一直占着目录，后到的等锁超时报"目录被占用"。
  // 改用跨执行存活的 ref 去重：创建请求只发一次；"已卸载"标志也放 ref 上——假卸载
  //（StrictMode cleanup）后 effect 立刻重跑会把它翻回 false，真卸载则保持 true。
  useEffect(() => {
    const startKey = `${projectId}\u0000${authProfileId}\u0000${targetUrlId}`
    if (startKeyRef.current !== startKey) {
      startKeyRef.current = startKey
      startPromiseRef.current = null
    }
    startDisposedRef.current = false
    startPromiseRef.current ??= request<AuthLoginSandboxSession>(apiRoutes.authLoginSandbox.create(), {
      method: "POST",
      body: JSON.stringify({ projectId, authProfileId, targetUrlId }),
    })
    startPromiseRef.current
      .then((res) => {
        if (startDisposedRef.current) {
          // 真卸载后响应才到：收尾关闭服务端浏览器（cancel 幂等，StrictMode 下重复调用无害）
      ...
      .catch((err) => {
        if (!startDisposedRef.current) setError(err instanceof Error ? err.message : String(err))
      })
    return () => {
      startDisposedRef.current = true
    }

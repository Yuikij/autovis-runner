...
export type ProfileFormState = {
  ...
  /** 前置登录态：手动登录沙盒启动时先注入这些 profile 的 storageState。 */
  prerequisiteAuthProfileIds: string[]
}

export const emptyFormState = (): ProfileFormState => ({ name: "", description: "", sourceCaseId: "", usePersistentProfile: true, prerequisiteAuthProfileIds: [] })
...
export function useAuthProfilesState(controller: ReadyWorkspaceController) {
  ...
  const handleSubmitForm = async (form: ProfileFormState) => {
    ...
    await saveAuthProfile({
      ...
      usePersistentProfile: form.usePersistentProfile,
      prerequisiteAuthProfileIds: form.prerequisiteAuthProfileIds,
    })
    setShowForm(false)
  }

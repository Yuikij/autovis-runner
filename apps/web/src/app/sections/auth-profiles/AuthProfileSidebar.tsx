  ...
  return (
    <aside className="space-y-2 lg:sticky lg:top-4">
      <div className="flex items-center justify-between px-1">
        <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{t("auth.profileListTitle")}</span>
      ...
      <div className="space-y-1.5 max-h-[70vh] overflow-y-auto pr-1">
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <span className={`size-1.5 rounded-full ${hasAnyState ? "bg-emerald-500" : "bg-rose-500"}`} title={hasAnyState ? t("auth.hasStorageState") : t("auth.noStorageState")} />
                  <span className={`size-1.5 rounded-full ${hasScript ? "bg-blue-500" : "bg-amber-500"}`} title={hasScript ? t("auth.scriptGenerated") : t("auth.scriptNotGenerated")} />
                  <button
                    type="button"
              ...
              <p className="mt-1 text-[10px] text-muted-foreground font-mono">
                {t("auth.urlStateCountLabel", { count: profile.states.length })}
              </p>
            </div>
          )
        })}
        {profiles.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border/50 bg-card/30 py-10 text-center">
            ...
            <p className="mt-2 text-xs text-muted-foreground">{t("auth.emptyProfiles")}</p>
          </div>
        ) : null}
      </div>
    </aside>

    ...
    <>
      ...
      <div className="space-y-3 pb-4 border-b border-border/40">
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t("wb.currentCase")}</span>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-sm py-1 border-b border-dashed border-border/30">
            <span className="text-muted-foreground text-xs">{t("wb.testCaseLabel")}</span>
            <select 
            ...
            >
              <option value="">{t("wb.selectTestCasePlaceholder")}</option>
        ...
        <div className="text-xs text-muted-foreground pt-1">
          <span className="font-semibold text-foreground">{t("wb.expectedResultLabel")}</span>{selectedCase.expectedResult}
        </div>
      </div>

      {/* Mode Switcher */}
      <div className="space-y-3">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">{t("wb.runMode")}</span>
        ...
        {mode === "generate" ? (
          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">{t("wb.targetUrl")}</label>
              <select
              ...
              >
                <option value="">{t("wb.pleaseSelectTargetUrl")}</option>
                {targetUrls.map((url) => (
                  <option key={url.id} value={url.id}>{url.label} · {url.url}</option>
                ))}
              </select>
              <p className="text-[10px] text-muted-foreground">{t("wb.targetUrlHintBefore")}<code className="font-mono">getBaseUrl()</code>{t("wb.targetUrlHintAfter")}</p>
            </div>

            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">{t("wb.generateInstruction")}</span>
            <textarea 
              className={`${textareaClassName} h-24 text-xs resize-none bg-secondary/20 border-border/60 focus:bg-background`}
              disabled={agentRunning} 
              onChange={(event) => setPrompt(event.target.value)} 
              placeholder={t("wb.generateInstructionPlaceholder")} 
              value={prompt} 
            />
            {!isConnected && (
              <div className="rounded-xl border border-warning/30 bg-warning/10 px-3 py-2 text-[11px] text-amber-200">
                {t("wb.copilotNotConnected")}
              </div>
            )}
            {targetUrls.length === 0 && (
              <div className="rounded-xl border border-warning/30 bg-warning/10 px-3 py-2 text-[11px] text-amber-200">
                {t("wb.noTargetUrlsHint")}
              </div>
            )}
            <Button 
            ...
            >
              ...
              {agentRunning ? t("wb.generating") : !targetUrlId ? t("wb.selectTargetUrlFirst") : t("wb.generateScript")}
            </Button>
          </div>
        ) : (
          <div className="space-y-3">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block">{t("wb.recordingConfig")}</span>
            <div className="space-y-2">
              <div className="space-y-1">
                <label className="text-[10px] text-muted-foreground">{t("wb.recordingUrl")}</label>
                <div className="flex gap-2">
                  ...
                  >
                    <option value="">{t("wb.selectTargetUrl")}</option>
                    {targetUrls.map((url) => (
                      <option key={url.id} value={url.id}>{url.label} · {url.url}</option>
                    ))}
                  </select>
              ...
              {recorderRunning && (
                <div className="space-y-2 pt-2 border-t border-border/30">
                  <label className="text-[10px] text-muted-foreground">{t("wb.sendTextInput")}</label>
                  {lastRecordedTarget && (
                    <div className="rounded-lg border border-border/50 bg-secondary/20 px-2.5 py-2 text-[10px] text-muted-foreground">
                      {t("wb.currentInputTarget")}{lastRecordedTarget.selector ?? lastRecordedTarget.label ?? lastRecordedTarget.placeholder ?? lastRecordedTarget.text ?? (lastRecordedTarget.x != null && lastRecordedTarget.y != null ? t("wb.lastClickCoords", { x: lastRecordedTarget.x, y: lastRecordedTarget.y }) : t("wb.lastClickedElement"))}
                    </div>
                  )}
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder={t("wb.inputTextPlaceholder")}

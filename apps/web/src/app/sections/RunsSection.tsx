...
export function RunsSection({ controller }: RunsSectionProps) {
  ...
  const handleDeleteRun = async (runId: string) => {
    ...
    if (ok && activeRun?.id === runId) {
      setActiveRun(null)
      setViewMode("list")
    }
  }

  const handleDeleteRunArtifacts = async (runId: string) => {
    const ok = await deleteRunArtifacts(runId)
    if (ok && activeRun?.id === runId) {
      try {
        const refreshed = await request<ExecutionRun>(apiRoutes.runs.detail(runId))
        setActiveRun(refreshed.data)
      } catch {
        setActiveRun(null)
      }
    }
  ...
  if (viewMode === "list") {
    return (
      <RunsList
        ...
        onDeleteTemporaryRun={handleDeleteRun}
  ...
  return (
    <RunDetail
      ...
      onDeleteRunArtifacts={handleDeleteRunArtifacts}

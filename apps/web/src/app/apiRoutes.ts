...
export const apiRoutes = {
  ...
  projects: {
    ...
    dataTables: (projectId: string) => api(`${project(projectId)}/data-tables`),
    knowledgeTree: (projectId: string) => api(`${project(projectId)}/knowledge/tree`),
    knowledgeSearch: (projectId: string, q: string) => api(`${project(projectId)}/knowledge/search${qs({ q })}`),
    knowledgeFile: (projectId: string, path?: string) => api(`${project(projectId)}/knowledge/file${qs({ path })}`),
    knowledgeDir: (projectId: string) => api(`${project(projectId)}/knowledge/dir`),
    knowledgeEntry: (projectId: string, path: string) => api(`${project(projectId)}/knowledge/entry${qs({ path })}`),
    knowledgeMove: (projectId: string) => api(`${project(projectId)}/knowledge/move`),
    knowledgeRaw: (projectId: string, path: string) =>
      api(`${project(projectId)}/knowledge/raw/${path.split("/").map(enc).join("/")}`),
  ...
  testCases: {
    ...
    contract: (testCaseId: string) => api(`${testCase(testCaseId)}/contract`),
    contractGenerate: (testCaseId: string) => api(`${testCase(testCaseId)}/contract/generate`),
    apiEnabled: (testCaseId: string) => api(`${testCase(testCaseId)}/api-enabled`),
    apiIntended: (testCaseId: string) => api(`${testCase(testCaseId)}/api-intended`),
    invoke: (testCaseId: string) => api(`${testCase(testCaseId)}/invoke`),
    apiDoc: (testCaseId: string) => api(`${testCase(testCaseId)}/api-doc`),
  },

  scripts: {
    generate: () => api("/scripts/generate"),
    directExecute: () => api("/scripts/direct-execute"),
    rewriteChat: () => api("/scripts/rewrite/chat"),
    rewritePlan: () => api("/scripts/rewrite/plan"),
  },

  runs: {
    create: () => api("/runs"),
    detail: (runId: string) => api(run(runId)),
    remove: (runId: string) => api(run(runId)),
    artifacts: (runId: string) => api(`${run(runId)}/artifacts`),
  ...
  dataTables: {
    update: (tableId: string) => api(`/data-tables/${enc(tableId)}`),
    remove: (tableId: string) => api(`/data-tables/${enc(tableId)}`),
    columns: (tableId: string) => api(`/data-tables/${enc(tableId)}/columns`),
    column: (tableId: string, columnId: string) => api(`/data-tables/${enc(tableId)}/columns/${enc(columnId)}`),
    rows: (tableId: string, params?: { limit?: number; offset?: number }) =>
      api(`/data-tables/${enc(tableId)}/rows${qs(params)}`),
    row: (tableId: string, rowId: string) => api(`/data-tables/${enc(tableId)}/rows/${enc(rowId)}`),
  },

...
export interface AuthProfileRow {
  ...
  use_persistent_profile: number | null
  prerequisite_auth_profile_ids: string | null
  created_at: string
  updated_at: string
}
...
export const mapTestCase = (row: TestCaseRow): TestCase => ({
  ...
  contract: parseJson(row.contract, undefined as TestCase["contract"]),
  apiIntended: row.api_intended === 1,
...
export const mapAuthProfile = (row: AuthProfileRow, states: AuthProfileState[] = []): AuthProfile => ({
  ...
  states,
  // 默认开：列为 NULL（老库迁移前）或 1 都视为开，仅显式 0 为关。
  usePersistentProfile: row.use_persistent_profile !== 0,
  prerequisiteAuthProfileIds: parseJson(row.prerequisite_auth_profile_ids, [] as string[]),
  createdAt: row.created_at,
  updatedAt: row.updated_at,
})

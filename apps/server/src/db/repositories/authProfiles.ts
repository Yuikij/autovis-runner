  ...
  public upsert(profile: AuthProfile): AuthProfile {
    ...
    const stmt = this.db.prepare(`
      INSERT INTO auth_profiles (
        id, project_id, name, description, source_case_id, validation_script_id, validation_script, validation_script_generated_at, use_persistent_profile, prerequisite_auth_profile_ids, created_at, updated_at
      )
      VALUES (
        @id, @projectId, @name, @description, @sourceCaseId, @validationScriptId, @validationScript, @validationScriptGeneratedAt, @usePersistentProfile, @prerequisiteAuthProfileIds, @createdAt, @updatedAt
      )
      ON CONFLICT(id) DO UPDATE SET
        ...
        use_persistent_profile = excluded.use_persistent_profile,
        prerequisite_auth_profile_ids = excluded.prerequisite_auth_profile_ids,
    ...
    stmt.run({
      ...
      // 默认开：仅显式 false 存 0。
      usePersistentProfile: profile.usePersistentProfile === false ? 0 : 1,
      prerequisiteAuthProfileIds: profile.prerequisiteAuthProfileIds?.length
        ? JSON.stringify(profile.prerequisiteAuthProfileIds)
        : null,

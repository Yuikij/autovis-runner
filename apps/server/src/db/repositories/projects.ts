 ...
 * 注意：本库 `PRAGMA foreign_keys = OFF`，外键 ON DELETE CASCADE 不会触发，所有子表都要在此手动删除，
 * 否则会残留孤儿数据（如项目删了但 data_tables / tasks / task_runs / schedule_triggers 还在）。
 * 顺序按「子→父」，且引用 task/task_run 的表要在其被删前先删。
 */
export const deleteProject = (db: DatabaseSync, projectId: string): string[] => {
  ...
  db.exec("BEGIN")
  try {
    // 任务控制指令 / 租约按 task_id 关联（无 project_id 列），趁 tasks / task_runs 还在时先按其 id 清掉。
    db.prepare("DELETE FROM task_control_commands WHERE task_id IN (SELECT id FROM tasks WHERE project_id = ?) OR task_id IN (SELECT id FROM task_runs WHERE project_id = ?)").run(projectId, projectId)
    db.prepare("DELETE FROM task_leases WHERE task_id IN (SELECT id FROM tasks WHERE project_id = ?) OR task_id IN (SELECT id FROM task_runs WHERE project_id = ?)").run(projectId, projectId)
    // agent / recorder / 运行记录
    ...
    // 任务与定时触发
    db.prepare("DELETE FROM schedule_triggers WHERE project_id = ?").run(projectId)
    db.prepare("DELETE FROM tasks WHERE project_id = ?").run(projectId)
    // 鉴权档案及其校验任务 / 状态
    db.prepare("DELETE FROM validation_tasks WHERE profile_id IN (SELECT id FROM auth_profiles WHERE project_id = ?)").run(projectId)
    ...
    // 项目数据表（列 / 行 → 表）
    db.prepare("DELETE FROM data_table_rows WHERE table_id IN (SELECT id FROM data_tables WHERE project_id = ?)").run(projectId)
    db.prepare("DELETE FROM data_table_columns WHERE table_id IN (SELECT id FROM data_tables WHERE project_id = ?)").run(projectId)
    db.prepare("DELETE FROM data_tables WHERE project_id = ?").run(projectId)
    // 用例与脚本

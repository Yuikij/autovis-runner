...
export async function runsRoutes(app: FastifyInstance) {
  ...
  app.delete("/runs/:runId", async (request, reply): Promise<ApiEnvelope<boolean> | { message: string }> => {
    const params = z.object({ runId: z.string() }).parse(request.params)
    const result = await store.deleteRun(params.runId)
    if (result === "not_found") {
      reply.code(404)
    ...
    if (result === "active") {
      reply.code(409)
      return { message: "运行尚未结束，请先停止任务后再删除记录。" }
    }
    return { data: true }
  })

  app.delete("/runs/:runId/artifacts", async (request, reply): Promise<ApiEnvelope<ExecutionRun> | { message: string }> => {
    const params = z.object({ runId: z.string() }).parse(request.params)
    const result = await store.deleteRunArtifacts(params.runId)
    if (result === "not_found") {
      reply.code(404)
      return { message: "Run not found" }
    }

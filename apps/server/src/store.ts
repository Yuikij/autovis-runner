...
class PersistentStore {
  ...
  // -- DataTable（项目级数据表）--
  async listDataTables(projectId: string) { return this.db.listDataTables(projectId) }
  async getDataTable(tableId: string) {
    const table = this.db.getDataTable(tableId)
    if (!table) throw new Error("数据表不存在")
    return table
  }
  async createDataTable(input: { projectId: string; name: string; description?: string }) {
    return this.db.createDataTable(input)
  }
  async updateDataTable(tableId: string, patch: { name?: string; description?: string }) {
    return this.db.updateDataTable(tableId, patch)
  }
  async deleteDataTable(tableId: string) { return this.db.deleteDataTable(tableId) }
  async addDataTableColumn(tableId: string, input: { name: string; type: DataColumnType }) {
    return this.db.addDataTableColumn(tableId, input)
  }
  async updateDataTableColumn(columnId: string, patch: { name?: string; type?: DataColumnType }) {
    return this.db.updateDataTableColumn(columnId, patch)
  }
  async deleteDataTableColumn(columnId: string) { return this.db.deleteDataTableColumn(columnId) }
  async listDataTableRows(tableId: string, options?: { limit?: number; offset?: number }) {
    return this.db.listDataTableRows(tableId, options)
  }
  async insertDataTableRow(tableId: string, data: Record<string, unknown>) {
    return this.db.insertDataTableRow(tableId, data)
  }
  async updateDataTableRow(rowId: string, data: Record<string, unknown>) {
    return this.db.updateDataTableRow(rowId, data)
  }
  async deleteDataTableRow(rowId: string) { return this.db.deleteDataTableRow(rowId) }
  ...
  // -- Case API (contract / 暴露为 API) --
  async updateTestCaseContract(testCaseId: string, contract: TestCase["contract"] | null) {
    if (!this.db.getTestCase(testCaseId)) throw new Error("用例不存在或已被删除。")
    return this.db.updateTestCaseContract(testCaseId, contract)
  }
  async setTestCaseApiEnabled(testCaseId: string, enabled: boolean) {
    const testCase = this.db.getTestCase(testCaseId)
    if (!testCase) throw new Error("用例不存在或已被删除。")
    if (enabled && !testCase.contract) throw new Error("请先定义 API 契约（contract）再开启 API。")
    return this.db.setTestCaseApiEnabled(testCaseId, enabled)
  }
  async setTestCaseApiIntended(testCaseId: string, intended: boolean) {
    const testCase = this.db.getTestCase(testCaseId)
    if (!testCase) throw new Error("用例不存在或已被删除。")
    return this.db.setTestCaseApiIntended(testCaseId, intended)
  }
  async invokeCase(input: { testCaseId: string; params?: Record<string, unknown>; targetUrlId?: string }) {
    return this.runService.invokeCase(input)
  }
  ...
  pauseRun(runId: string) { return this.recordTaskControlCommand("run", runId, "pause") || this.forwardRunControlToOwningAgent(runId, "pause") }
  resumeRun(runId: string) { return this.recordTaskControlCommand("run", runId, "resume") || this.forwardRunControlToOwningAgent(runId, "resume") }
  cancelRun(runId: string) { return this.recordTaskControlCommand("run", runId, "cancel") || this.forwardRunControlToOwningAgent(runId, "cancel") }

  /**
   * Agent 的 warmup / latest run 并没有独立的 run 任务租约（它们由 agent 会话持有），
   * 因此直接对其下达 run 控制命令会被判为 "Run not cancellable"。这里回退：找到拥有该 run 的
   * 活跃 agent，把控制命令转发到 agent 上，让前端对 warmup run 点「停止/暂停」也能生效。
   */
  private forwardRunControlToOwningAgent(runId: string, action: TaskControlAction): boolean {
    const owningAgent = this.db
      .listAllAgentSessions()
      .find((session) => (session.warmupRunId === runId || session.latestRunId === runId) && this.isActiveAgentStatus(session.status))
    if (!owningAgent) return false
    return this.recordTaskControlCommand("agent", owningAgent.id, action)
  }

  /** 删除单条执行记录及其磁盘产物。运行未结束时拒绝，避免误删进行中的任务。 */
  async deleteRun(runId: string): Promise<"deleted" | "not_found" | "active"> {
    const run = this.db.getRun(runId)
    if (!run) return "not_found"
    if (this.isRunActive(run)) return "active"
    this.db.deleteRun(runId)
    await removeArtifactDirs([runId])
    return "deleted"
  }

  /** 仅删除某条运行的磁盘产物（截图、视频、trace 等），保留执行记录本身。 */
  async deleteRunArtifacts(runId: string): Promise<ExecutionRun | "not_found" | "active"> {
    const run = this.db.getRun(runId)
    if (!run) return "not_found"
    if (this.isRunActive(run)) return "active"
    await removeArtifactDirs([runId])
    const cleaned: ExecutionRun = {
      ...run,
      artifacts: [],
      currentViewport: "",
      liveViewport: undefined,
      steps: run.steps.map((step) => ({ ...step, screenshotUrl: undefined })),
    }
    this.db.upsertRun(cleaned)
    this.runStateService.notifyRun(cleaned)
    return cleaned
  }

  private isRunActive(run: ExecutionRun) {
    if (this.tasks.has(run.id)) return true
    return run.status === "queued" || run.status === "running" || run.status === "paused" || run.status === "cancelling" || run.status === "awaiting_human"
  }

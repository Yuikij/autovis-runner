...
export class TaskRunService {
  ...
  private createManagedTaskRunController(
    ...
    opts: {
      ...
      /** deadline 目标时刻（ISO），来自调度器按触发器时间注入；缺省表示无目标、不卡点。 */
      deadlineAt?: string
    },
  ) {
  ...
  private shouldContinueAfterFailure(item: Task["items"][number]): boolean {
    return item.onFailure === "continue"
  }
      ...
      try {
        for (let index = startIndex; index < resolvedItems.length; index += 1) {
          ...
          if (!testCase.latestScriptId) {
            if (this.runDirectAgentForTask) {
              ...
              if (agentFailed && !taskController.signal.aborted) {
                if (this.shouldContinueAfterFailure(item)) {
                  taskRun.logs.push(`${testCase.caseCode} 配置为失败后继续，继续执行后续步骤。`)
                  this.persistAndNotifyTaskRun(taskRun)
                } else {
          ...
          const run = await this.runService.startRun({
            ...
            // deadline：把目标时刻透传到子 run，由 runner 在脚本正文前自动卡点。
            // 首个用例会真正等待到目标时刻；后续用例时目标已过，waitUntil 为无操作。
            deadlineWaitUntil: opts.effectiveTaskMode.kind === "deadline" ? opts.deadlineAt : undefined,
            chain: {
  ...
  public async recoverTaskRun(taskRunId: string): Promise<TaskRun | undefined> {
    ...
    const deadlineAt = typeof leaseRequest.deadlineAt === "string" ? leaseRequest.deadlineAt : undefined
    const opts = {
      ...
      effectiveTaskMode,
      deadlineAt,
      scriptTimeoutMs: typeof leaseRequest.scriptTimeoutMs === "number" ? leaseRequest.scriptTimeoutMs : this.computeScriptTimeoutMsForMode(effectiveTaskMode, deadlineAt),
    }
    ...
    // 恢复路径保留失败策略：只有最近完成的失败步骤仍采用默认 stop 时，才补齐快速失败跳过。
    if (taskRun.failedCount > 0 && startIndex < resolvedItems.length) {
      const lastCompletedItem = resolvedItems[startIndex - 1]?.item
      if (!lastCompletedItem || !this.shouldContinueAfterFailure(lastCompletedItem)) {
  ...
  public async startTaskRun(request: StartTaskRunRequest): Promise<TaskRun> {
    ...
    return this.runTaskRunOnce(task, {
      ...
      scriptTimeoutMs: this.computeScriptTimeoutMsForMode(effectiveTaskMode, request.deadlineAt),
    })
  }

  /**
   * deadline 目标时刻来自调度器注入的 deadlineAt（触发器时间），不再由模式自带。
   * 无 deadlineAt（如手动立即执行）时返回 undefined = 用 runner 默认超时，不卡点。
   */
  private computeScriptTimeoutMsForMode(mode: TaskModeConfig, deadlineAt?: string): number | undefined {
    if (mode.kind === "deadline") {
      const targetMs = deadlineAt ? Date.parse(deadlineAt) : NaN
      if (Number.isFinite(targetMs)) {
        const remaining = Math.max(0, targetMs - Date.now())
        return remaining + DEFAULT_DEADLINE_POST_WINDOW_MS
      }
      return undefined
    }
  ...
  private async runTaskRunOnce(
    ...
    taskRun.logs.push("任务执行链：跳过用例自带前置；默认任一用例失败即快速失败终止；单个步骤可配置为失败后继续。")
    ...
    if (opts.effectiveTaskMode.kind === "deadline") {
      taskRun.logs.push(
        opts.deadlineAt
          ? `deadline · 已提前启动并完成浏览器/登录/首页预热，将自动卡到目标时刻 ${opts.deadlineAt} 再执行脚本正文。`
          : `deadline · 手动立即执行（无触发器目标时刻），按普通预热执行、不卡点。`,
      )
    }

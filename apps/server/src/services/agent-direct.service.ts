...
export class AgentDirectService {
  ...
  /**
   * 从 agent 步骤里提取报告卡片（payloadJson 里的 savedReport）。
   * 凡是通过 writeReportArtifact 产报告的工具都会带 savedReport（save_report、translate_document……），
   * 所以按 payloadJson 内容识别、不按 toolName 白名单，避免漏掉 translate_document 等产物进不了收件箱。
   */

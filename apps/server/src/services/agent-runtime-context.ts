...
import { now, resolvePersistentProfileDir } from "./common.js"
...
export async function closeWarmupSession(warmupSession: RunnerSession | null) {
  ...
  if (!warmupSession.persistent) {
    await warmupSession.browser?.close().catch(() => undefined)
  }
  warmupSession.releaseLock?.()
}
...
export async function prepareAgentExecutionContext(params: {
  ...
  /** 任务取消信号：透传给 warmup，让「停止」能打断启动浏览器 / 前置依赖阶段。 */
  signal?: AbortSignal
  ...
  let authUserDataDir: string | undefined
  if (testCase.authProfileId && resolvedRunTarget.id) {
    ...
    if (authProfile) {
      if (authProfile.usePersistentProfile !== false) {
        authUserDataDir = resolvePersistentProfileDir(authProfile.id)
      }

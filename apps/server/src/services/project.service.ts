...
export class ProjectService {
  ...
  public async deleteProject(projectId: string) {
    ...
    await knowledgeService.removeProject(projectId)
  ...
  public async deleteTestCase(testCaseId: string) {
    ...
    // 检查是否有登录态配置引用该用例
    const authProfiles = this.db.listAuthProfiles(targetCase.projectId)
    const referencingProfiles = authProfiles.filter(profile => profile.sourceCaseId === testCaseId)
    if (referencingProfiles.length > 0) {
      throw new Error(`该测试用例被以下登录态配置引用，无法删除：${referencingProfiles.map((item) => item.name).join(", ")}。请先删除相关登录态配置或修改其源用例。`)
    }
  ...
  public async saveAuthProfile(input: UpsertAuthProfileRequest) {
    ...
    const id = input.id ?? createId("auth_profile")
    // 前置登录态：去掉自引用与不属于本项目的 id，保留用户配置顺序（即注入顺序）。
    const projectProfileIds = new Set(this.db.listAuthProfiles(input.projectId).map((profile) => profile.id))
    const prerequisiteAuthProfileIds = (input.prerequisiteAuthProfileIds ?? existing?.prerequisiteAuthProfileIds ?? [])
      .filter((profileId, index, list) => profileId !== id && projectProfileIds.has(profileId) && list.indexOf(profileId) === index)
    return this.db.upsertAuthProfile({
      ...
      usePersistentProfile: input.usePersistentProfile ?? existing?.usePersistentProfile ?? true,
      prerequisiteAuthProfileIds,

...
export class TaskService {
  ...
  private normalizeItems(projectId: string, items: TaskItem[]): TaskItem[] {
    ...
    return items.map((item, index) => {
      ...
      const onFailure = item.onFailure === "continue" ? "continue" : undefined
      const stealth = typeof item.stealth === "boolean" ? item.stealth : undefined
      // 首个用例总是全新会话；仅在 index>0 且显式开启时保留续用标记。
      const continueSession = index > 0 && item.continueSession === true ? true : undefined
      return { caseId, targetUrlId, onFailure, continueSession, stealth }
    })
  }

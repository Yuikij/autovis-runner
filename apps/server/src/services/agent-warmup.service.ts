...
export type WarmupOptions = {
  ...
  /** 持久 profile 的 userDataDir（开了持久模式时）：warmup 浏览器复用同一目录。 */
  ...
  /** 任务取消信号：让「停止」能打断 warmup 阶段（启动浏览器 / 执行前置依赖），而不是只在 agent 主循环生效。 */
  signal?: AbortSignal
  /** 暂停闸门：warmup 各阶段开始前 await。 */
...
export class AgentWarmupService {
  ...
  public async executeWarmup(options: WarmupOptions): Promise<ExecuteWarmupResult> {
    const { sessionId, mode, taskRunId, project, testCase, resolvedRunTargetId, resolvedRunUrl, authStorageStateJson, authUserDataDir, stealth, provider, llmOwnerKey, onStep, updateSession, signal, waitIfPaused } = options
    ...
    try {
      ...
      for (const dependency of preconditionPlan.nodes) {
        ...
        await executeScriptInSession({
          ...
          knowledge: knowledgeService.createScriptApi(project.id),
        })
      }
      ...
      if (currentWarmupRunId) {
        ...
        if (run) {
          run.status = (isBrowserMissing || wasCancelled) ? "cancelled" : "failed"
      ...
      onStep({
        ...
        type: "error",
        stage: "page",
        title: isBrowserMissing ? "浏览器未安装" : wasCancelled ? "已停止" : "前置依赖预热失败",

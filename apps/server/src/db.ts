...
import {
  ...
  listTasks,
  replaceAgentSteps,
  updateTestCaseContract,
  setTestCaseApiEnabled,
  setTestCaseApiIntended,
  saveLlmState,
  saveLlmConfigState,
...
export class BrowsewrightDatabase {
  ...
  private readonly dataTablesRepo: DataTableRepository
  ...
  constructor(private readonly dataDir: string, appOrigin: string) {
    ...
    this.dataTablesRepo = new DataTableRepository(this.db)
  ...
  listDataTables(projectId: string): DataTable[] {
    return this.dataTablesRepo.listByProject(projectId)
  }

  getDataTable(tableId: string): DataTable | undefined {
    return this.dataTablesRepo.getTable(tableId) ?? undefined
  }

  createDataTable(input: { projectId: string; name: string; description?: string }): DataTable {
    return this.dataTablesRepo.createTable(input)
  }

  updateDataTable(tableId: string, patch: { name?: string; description?: string }): DataTable {
    return this.dataTablesRepo.updateTable(tableId, patch)
  }

  deleteDataTable(tableId: string): void {
    this.dataTablesRepo.deleteTable(tableId)
  }

  addDataTableColumn(tableId: string, input: { name: string; type: DataColumnType }): DataTable {
    return this.dataTablesRepo.addColumn(tableId, input)
  }

  updateDataTableColumn(columnId: string, patch: { name?: string; type?: DataColumnType }): DataTable {
    return this.dataTablesRepo.updateColumn(columnId, patch)
  }

  deleteDataTableColumn(columnId: string): DataTable {
    return this.dataTablesRepo.deleteColumn(columnId)
  }

  listDataTableRows(tableId: string, options?: { limit?: number; offset?: number }): DataTableRowPage {
    return this.dataTablesRepo.listRows(tableId, options)
  }

  insertDataTableRow(tableId: string, data: Record<string, unknown>): DataTableRow {
    return this.dataTablesRepo.insertRow(tableId, data)
  }

  updateDataTableRow(rowId: string, data: Record<string, unknown>): DataTableRow {
    return this.dataTablesRepo.updateRow(rowId, data)
  }

  deleteDataTableRow(rowId: string): void {
    this.dataTablesRepo.deleteRow(rowId)
  }

  /** 构建绑定到某项目的脚本运行时 data-tables API（供 run.service / agent 注入沙箱）。 */
  createDataTableScriptApi(projectId: string, onLog?: (line: string) => void): DataTableScriptApi {
    return this.dataTablesRepo.createScriptApi(projectId, onLog)
  }
  ...
  updateTestCaseContract(testCaseId: string, contract: TestCase["contract"] | null) {
    return updateTestCaseContract(this.db, testCaseId, contract ?? null)
  }

  setTestCaseApiEnabled(testCaseId: string, enabled: boolean) {
    return setTestCaseApiEnabled(this.db, testCaseId, enabled)
  }

  setTestCaseApiIntended(testCaseId: string, intended: boolean) {
    return setTestCaseApiIntended(this.db, testCaseId, intended)
  }

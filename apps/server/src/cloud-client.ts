...
// 被同 token 的其他 runner 抢占（云端 1012 "replaced"）后的重连退避：
// 比普通重连长，避免两个实例 1s 级互踢的"抢占风暴"，同时不至于像分钟级那样把自己长时间下线。
const REPLACED_BACKOFF_MS = 15_000
  ...
  const connect = async () => {
    while (!stopped) {
      try {
        ...
        if (openedThisCycle && (disconnectCode === 1012 || /replaced/i.test(disconnectReason ?? ""))) {
          reconnectDelay = Math.max(reconnectDelay, REPLACED_BACKOFF_MS)
          log.warn("relay.replaced_by_other_device", {
            deviceId,
            cloudUrl,
            code: disconnectCode,
            reason: disconnectReason,
            backoffMs: reconnectDelay,
            hint: "同一 device token 被另一个 runner 抢占。一个 token 只应跑一个 runner——请停掉重复实例或为本机重新签发 token。已拉长重连退避以避免抢占风暴。",
          })
        }

...
export function buildAgentSystemPrompt(): string {
  return [
    ...
    "- `outputs.add(description, value, meta?)`：当前节点给下游节点的结构化输出。新增/修改/删除/选择到后续会用的数据时，在相关步骤成功后调用。",
    ...
    "- `retry(fn, { times, backoffMs?, backoffFactor?, shouldRetry?, description? })`：失败重试。下单 / 提交按钮被反作弊拒绝时用它，比 try/catch 重复粘贴可读且自带退避。",
    ...
    "  2. **人机验证 / 登录墙**（出现验证码、跳到登录页；快照顶部会有 `[⚠️ ...]` 提示）→ `await human.input({ reason: 'captcha', instruction: '请在浏览器完成验证/登录后继续' })`；仍不行就输出文本说明并停手，不要无脑重试。",

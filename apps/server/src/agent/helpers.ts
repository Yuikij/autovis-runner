...
/**
 * 在页面**主世界**注入一个 `__name` 空实现兜底。
 *
 * 背景：dev 下 server 用 tsx（esbuild `keepNames`）运行，会把传给 `page.evaluate` 的函数里的具名内部函数
 * 改写成 `__name(fn, "fn")`，但浏览器没有这个 helper，evaluate 时抛 `ReferenceError: __name is not defined`。
 * 注意平台用 patchright（stealth），`addInitScript` 跑在隔离世界，设的全局在主世界看不到；因此必须在**同一文档、
 * 主世界**里、紧挨着目标 evaluate 之前用 `page.evaluate(字符串)` 注入（裸标识符 `__name` 会沿作用域链落到 globalThis）。
 * 生产 tsc 构建不注入该引用，此 shim 无副作用。任何含具名内部函数的 `page.evaluate` 之前都可调用它。
 */
export async function ensureEvaluateNameShim(page: Page | Frame): Promise<void> {
  await page.evaluate("globalThis.__name = globalThis.__name || function (f) { return f };").catch(() => undefined)
}
...
export function buildToolTitle(toolName: string, args: Record<string, unknown>): string {
  switch (toolName) {
    ...
    case "probe_step": return `探针实验 · ${String(args.title ?? "")}`.trim()

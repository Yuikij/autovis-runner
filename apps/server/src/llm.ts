...
import { recordLlmFailure } from "./observability.js"
...
export const generateScriptWithLlm = async (context: ScriptGenerationContext & { secrets: LlmSecretState }) => {
  try {
    ...
    recordLlmFailure(context.session.provider, "generate_script", error, {
      model: context.session.model,
    })
    throw error
  }
...
export const generateValidationScriptWithLlmV2 = async (
  ...
  try {
    ...
    recordLlmFailure(context.session.provider, "generate_validation_script", error, {
      model: context.session.model,
    })
    throw error
  }
...
export const analyzeImageWithLlm = async (
  ...
  try {
    ...
    recordLlmFailure(input.session.provider, "analyze_image", error, {
      model: input.session.model,
    })
    throw error
  }
...
export const generateTextWithLlm = async (input: {
  ...
  const result = await callLlmWithTools(input.session, input.secrets, messages, [], input.signal)
...
export const callLlmWithTools = async (
  ...
  try {
    if (session.provider === "copilot-proxy") {
      return callCopilotWithTools(session, secrets.copilot ?? {}, messages, tools, signal)
    }

    if (session.provider === "openai-compatible") {
      return callOpenAiWithTools(session, secrets, messages, tools, signal)
    }

    if (session.provider === "anthropic-compatible") {
      return callAnthropicWithTools(session, secrets, messages, tools, signal)
    }

    throw new Error(`不支持的 LLM provider: ${session.provider}`)
  } catch (error) {
    recordLlmFailure(session.provider, "tool_call", error, {
      model: session.model,
